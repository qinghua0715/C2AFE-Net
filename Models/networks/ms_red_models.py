...
class HSBlock(nn.Module):
    ...
    def __init__(self, in_ch, s=4):
        ...
        # 添加一个空序列，因为只有分支的第一组可以不用任何操作直接到下一层
    ...
    def forward(self, x):
        x = list(x.chunk(chunks=self.s, dim=1))  # 在给定维度(轴)上将输入张量进行分块儿,即在通道方向分块
...
class ASFF_ddw(nn.Module):
    def __init__(self, level, rfb=False, vis=False):
        ...
        if level == 0:
            self.stride_level_1 = add_conv(128, self.inter_dim, 3, 2)    # 128 56 80 -> 256 28 40
            self.stride_level_2 = nn.Sequential(
                add_conv(64, self.inter_dim, 3, 2),    # 64 112 160 -> 256 56 80
                nn.MaxPool2d(kernel_size=2, stride=2)    # 256 56 80 -> 256 28 40
            )
            self.stride_level_3 = nn.Sequential(
                add_conv(32, 64, 3, 2),       # 32 224 320 -> 64 112 160
                add_conv(64, self.inter_dim, 3, 2),         # 64 112 160 -> 256 56 80
                nn.MaxPool2d(kernel_size=2, stride=2)       # 256 56 80 -> 256 28 40
            )
            self.expand = add_conv(self.inter_dim, 256, 3, 1)  # ????   # 256 28 40 -> 256 28 40
    ...
    def forward(self, x_level_0, x_level_1, x_level_2, x_level_3):
        if self.level == 0:
            level_0_resized = x_level_0     # 256 28 40
            level_1_resized = self.stride_level_1(x_level_1)        # 128 56 80 -> 256 28 40
            level_2_resized = self.stride_level_2(x_level_2)        # 64 112 160 -> 256 28 40
            level_3_resized = self.stride_level_3(x_level_3)        # 32 224 320 -> 256 28 40
        ...
        level_0_weight_v = self.weight_level_0(level_0_resized)         # 256 28 40 -> 16 28 40
        level_1_weight_v = self.weight_level_1(level_1_resized)         # 256 28 40 -> 16 28 40
        level_2_weight_v = self.weight_level_2(level_2_resized)          # 256 28 40 -> 16 28 40
        level_3_weight_v = self.weight_level_3(level_3_resized)          # 256 28 40 -> 16 28 40
        # 16*4 28 40
        levels_weight_v = torch.cat((level_0_weight_v, level_1_weight_v, level_2_weight_v, level_3_weight_v), 1)
        levels_weight = self.weight_levels(levels_weight_v)     # 16*4 28 40 -> 4 28 40
        levels_weight = F.softmax(levels_weight, dim=1)         # 4 28 40
        # (256 28 40 * 1 28 40 ) × 4 -> 256 28 40
        fused_out_reduced = level_0_resized * levels_weight[:, 0:1, :, :] + \
                            level_1_resized * levels_weight[:, 1:2, :, :] + \
                            level_2_resized * levels_weight[:, 2:3, :, :] + \
                            level_3_resized * levels_weight[:, 3:, :, :]

        out = self.expand(fused_out_reduced)    # 256 28 40 -> 256 28 40
...
class Ms_red_v1(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.out_size = (224, 224)    # (224, 320)
        self.enc_input = ResEncoder_hs(in_channels, 32)
    ...
    def forward(self, x):
        ...
        out1 = self.scale_att(dsv_cat)  # [16, 4, 224, 300]

        out2 = self.final(out1)

        final = torch.sigmoid(out2)
        return final
